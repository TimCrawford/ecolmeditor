...
if($_POST['apply']) {
  ...
  echo json_encode(reset_password($_POST['reset']));
} else if($_POST['survey']){
  echo json_encode(survey_users());
} else if ($_POST['review']){
  echo json_encode(review_applications());
} else if ($_POST['approve']){
  echo json_encode(approve_application($_POST['approve'], $_POST['email']));
} else if ($_POST['username'] && $_POST['password']){
  echo json_encode(add_user($_POST['username'], $_POST['fullname'], $_POST['password'],
                            $_POST['country'], $_POST['e-mail']));
  random_allocation($_POST['username'], 2, "all");
} else if($_POST['spyon']){
  echo json_encode(user_assignments_super($_POST['spyon']));
} else if($_POST['sysCheck']){
  echo json_encode(all_allocations_please());
} else if($_POST['sync']){
  echo json_encode(sync_assignment_super($_POST['username'], $_POST['id'],
