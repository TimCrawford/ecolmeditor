    ...
    if (config) {
        ...
        return {
            ...
            save_small: function() {
                window.open("data:audio/midi;base64," + this.b64,
                            "JSMidi generated output",
                            "resizable=no,scrollbars=no,status=no,width=250,height=20");
           }
        };
...
/**
 * Returns an event of the type PROGRAM_CHANGE taking the values passed and falling
 * back to defaults if they are not specified.
 *
 */
/*  DOESN'T WORK OF COURSE
MidiEvent.program_change = function(program_change) {
    return new MidiEvent({
        type:    EVT_PROGRAM_CHANGE,
        param1:  program_change  || 0
    });
};
*/
